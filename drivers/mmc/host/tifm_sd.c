			      ...
			      unsigned int off, unsigned int cnt)
{
	...
	buf = sg_kmap_offset(sg, off - sg->offset, SG_KMAP_ATOMIC);
	if (IS_ERR(buf)) {
		/*
	...
	sg_kunmap_offset(sg, buf, off - sg->offset, SG_KMAP_ATOMIC);
}
			      ...
			      unsigned int count)
{
	...
	src_buf = sg_kmap_offset(src, src_off, SG_KMAP_ATOMIC);
	if (IS_ERR(src_buf))
		goto sg_kmap_err;

	dst_buf = sg_kmap_offset(dst, dst_off, SG_KMAP_ATOMIC);
	if (IS_ERR(dst_buf))
		goto sg_kmap_err;

	memcpy(dst_buf, src_buf, count);

	sg_kunmap_offset(dst, dst_buf, dst_off, SG_KMAP_ATOMIC);
	sg_kunmap_offset(src, src_buf, src_off, SG_KMAP_ATOMIC);
