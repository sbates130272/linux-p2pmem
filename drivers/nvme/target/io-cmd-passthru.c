		...
		struct request *rq, u16 status)
{
	...
	if (req->p.passthru_ns) {
		nvme_put_ns(req->p.passthru_ns);
		req->p.passthru_ns = NULL;
	}
}
...
static void nvmet_passthru_execute_cmd(struct nvmet_req *req)
{
	...
	rq = nvmet_passthru_blk_make_request(req, first_bio, GFP_KERNEL);
	if (IS_ERR(rq))
		goto fail_free_bio;

	if (blk_rq_nr_phys_segments(rq) > queue_max_segments(rq->q) ||
	    (blk_rq_payload_bytes(rq) >> 9) > queue_max_hw_sectors(rq->q)) {
		nvmet_passthru_req_complete(req, rq, NVME_SC_INVALID_FIELD);
		return;
	}
...
static bool nvmet_parse_passthru_io_cmd(struct nvmet_req *req)
{
	u32 nsid;

	/*
	 * XXX: allow all I/O commands for now, in future check here for the
	 *      namespace type and the supported I/O command set.
	 */
	nsid = le32_to_cpu(req->cmd->common.nsid);
	req->p.passthru_ns = nvme_find_get_ns(nvmet_req_passthru_ctrl(req),
					      nsid);
	if (unlikely(!req->p.passthru_ns)) {
		pr_err("failed to get passthru ns nsid:%u\n", nsid);
