	...
	for_each_sg(sg, s, nents, i) {
		...
		if (is_pci_p2pdma_page(sg_page(s)) && !s_iova_len) {
			if (i > 0)
				cur = sg_next(cur);
			/*
			 * The map_type was already checked earlier so this
			 * call is guaranteed to map the segment and there's
			 * no need to check the return value.
			 */
			pci_p2pdma_map_segment(&p2pdma_state, dev,
					       sg_page(s)->pgmap, cur, attrs);
			count++;
			cur_len = 0;
			continue;
		}
