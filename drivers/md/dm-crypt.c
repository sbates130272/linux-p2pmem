			    ...
			    struct dm_crypt_request *dmreq)
{
	...
	if (bio_data_dir(dmreq->ctx->bio_in) == WRITE) {
		src = sg_kmap(&dmreq->sg_in, SG_KMAP_ATOMIC);
		if (IS_ERR(src))
			return PTR_ERR(src);

		r = crypt_iv_lmk_one(cc, iv, dmreq, src);
		sg_kunmap(&dmreq->sg_in, src, SG_KMAP_ATOMIC);
	} else
		memset(iv, 0, cc->iv_size);
			     ...
			     struct dm_crypt_request *dmreq)
{
	...
	dst = sg_kmap(&dmreq->sg_out, SG_KMAP_ATOMIC);
	if (IS_ERR(dst))
		return PTR_ERR(dst);
	...
	sg_kunmap(&dmreq->sg_out, dst, SG_KMAP_ATOMIC);
	return r;
}
