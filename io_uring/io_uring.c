...
static inline bool __io_fill_cqe_req(struct io_kiocb *req)
{
	trace_io_uring_complete(req->ctx, req, req->cqe.user_data, req->cqe.res,
				req->cqe.flags, 0, 0);
	return __io_fill_cqe(req->ctx, req->cqe.user_data, req->cqe.res,
				req->cqe.flags);
}

static inline void __io_fill_cqe32_req(struct io_kiocb *req, u64 extra1,
				       u64 extra2)
{
	struct io_ring_ctx *ctx = req->ctx;
	struct io_uring_cqe *cqe;
	...
	io_cqring_event_overflow(ctx, req->cqe.user_data, req->cqe.res,
					req->cqe.flags, extra1, extra2);
}
...
{
	if (!(req->flags & REQ_F_CQE_SKIP))
		__io_fill_cqe_req(req);
	__io_req_complete_put(req);
}

static void __io_req_complete_post32(struct io_kiocb *req, u64 extra1,
				     u64 extra2)
{
	if (!(req->flags & REQ_F_CQE_SKIP))
		__io_fill_cqe32_req(req, extra1, extra2);
	__io_req_complete_put(req);
}
...
static void io_req_complete_post32(struct io_kiocb *req, u64 extra1, u64 extra2)
{
	struct io_ring_ctx *ctx = req->ctx;

	spin_lock(&ctx->completion_lock);
	__io_req_complete_post32(req, extra1, extra2);
	io_commit_cqring(ctx);
	spin_unlock(&ctx->completion_lock);
	io_cqring_ev_posted(ctx);
}
				       ...
				       u64 extra2)
{
	if (issue_flags & IO_URING_F_COMPLETE_DEFER) {
		...
		io_req_complete_post32(req, extra1, extra2);
	}
}
...
static int io_do_iopoll(struct io_ring_ctx *ctx, bool force_nonspin)
{
	...
	wq_list_for_each_resume(pos, prev) {
		...
		if (unlikely(req->flags & REQ_F_CQE_SKIP))
			continue;
		io_req_set_res(req, req->cqe.res, io_put_kbuf(req, 0));
		__io_fill_cqe_req(req);
	}
...
void io_uring_cmd_done(struct io_uring_cmd *ioucmd, ssize_t ret, ssize_t res2)
{
	...
	if (req->ctx->flags & IORING_SETUP_CQE32)
		__io_req_complete32(req, 0, res2, 0);
	else
...
static int io_close_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	close->flags = READ_ONCE(sqe->close_flags);
	if (close->flags & ~IORING_CLOSE_FD_AND_FILE_SLOT)
		return -EINVAL;
	if (!(close->flags & IORING_CLOSE_FD_AND_FILE_SLOT) &&
	    close->file_slot && close->fd)
		return -EINVAL;
